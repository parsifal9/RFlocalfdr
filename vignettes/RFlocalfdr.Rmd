...
a lot of variables and a lot of trees.
...
Load some test data (iris is too small). This data is already processed to this stage,
i.e. it is logged and scaled.
...
Get the fdr
```{r, eval=FALSE}
aa<-local.fdr(f,FUN=my.dsn,  xi=summary(mm1.df2)$parameters[1,1], omega=summary(mm1.df2)$parameters[2,1], lambda= summary(mm1.df2)$parameters[3,1],p0=p0 )
plot(x,aa)
abline(h=0.2)
ww<-which.min(abs(aa[20:119]-0.2))
...
abline(v=sn::qsn(0.95,  xi=summary(mm1.df2)$parameters[1,1], omega=summary(mm1.df2)$parameters[2,1], alpha= summary(mm1.df2)$parameters[3,1] ))
abline(v=x[as.numeric(names(ww))])
abline(v=C,col="red")
