...
1's than 0's.  We plot a small data set of $300\times  508$ to explore the data generation method.
...
library(Boruta)
#set.seed(120)  
#boruta1 <- Boruta(X,as.factor(y),  num.threads = 7)
#print(boruta1)
##Boruta performed 99 iterations in 20.27062 secs.
## 2 attributes confirmed important: X116, X6142;
## 6341 attributes confirmed unimportant: X1, X10, X100, X1000, X1001 and 6336 more;
## 7 tentative attributes left: X108, X34, X40, X45, X48 and 2 more;
#plotImpHistory(boruta1)
#aa<-which(boruta1$finalDecision=="Confirmed") # 116 6142
#bb<-which(boruta1$finalDecision=="Tentative")  # 34   40   45   48   50  108 5473
...
system(res <- var.sel.rfe(X, y, prop.rm = 0.2,  recalculate = TRUE, tol = 10, 
    ntree = 500, mtry.prop = 0.2, nodesize.prop = 0.1, no.threads = 7, 
    method = "ranger", type = "classification", importance = "impurity", 
    case.weights = NULL) )
 res$var
# [1] "X1"    "X108"  "X12"   "X13"   "X14"   "X18"   "X19"   "X2"    "X25"  
#[10] "X29"   "X3"    "X4"    "X5"    "X50"   "X53"   "X5469" "X6"    "X61"  
#[19] "X6135" "X62"   "X65"   "X7"    "X72"   "X81"  
tt<-as.numeric(gsub("X([0-9]*)","\\1", res$var))
table(ifelse((tt < 127),1,2))
# 1  2 
#22  2 0.0833

res<-c(1, 108,12, 13, 14, 18, 19, 2,25,29, 3,4,5,50, 53, 5469,6,61, 6135,62, 65, 7,72, 81)
...
- permutes the $S$ times 
- For each permutation of the response vector $y^{*s}$, a new RF is grown and the permutation
  variable importance measure (VarImp^{*s}) for all predictor variables X is computed. 
- The vector ‘perVarImp’ of $S$ VarImp measures for every predictor variables are used to approximate the null importance distributions (‘PimpTest’).
- the null importance distribution is calculated for each variable, we are doing $p$ tests
- hence a multiple testing correction is in order
...
- we have done thtat in our code and also 
...
system.time(pimp.varImp.cl<-my.ranger.PIMP(X,y,cl.ranger,S=10, parallel=TRUE, ncores=3)) 
...
conf_matrix<-table(predicted_values,c(rep(1,127),rep(0,6350-127)))
conf_matrix
sensitivity(conf_matrix) #0.910654 TP/(TP+FN)
...
table(ifelse((tt < 127),1,2))
# 1  2 
# 39 556      556 /(556+39) = 0.9344538


```{r eval=FALSE,echo=FALSE}
#jhow to make a neat html tablein Rmarkdown
library(tables)
X <- rnorm(125, sd=100)
