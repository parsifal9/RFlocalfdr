...
Variable importance is often used to perform variable selection. More precisely, it is possible to identify two final aims (Genuer et al., 2010): 
...
It is the second objective that is more often our aim in biomolecular work. In this area the problem is characterized by:
...
3. the selected variables are used as input for further analysis.

# Discussion and Conclusion

We simulate a classification problem with 6350 variables of which 127 are related to the classification and a high degree of correlation structure. 
...
obtained for these methods.  I could not get many of the SHAP implementations to converge in a 2 1/2 hour run time
limit. Shaff converged and returned one variable.
- where there is a method of thresholding the variable importance to do variable selection, we consider the accuracy of the selected set of variables.

The results are 
In the rest

Recursive Feature Elimination has  performed well but only selected a small number of variables.
I could not get many of the SHAP implementations to converge in a 2 1/2 hour run time
limit. Shaff converged and returned one variable.

```{r table3, echo=FALSE, message=FALSE, warnings=FALSE, eval=TRUE}
tabl <- "
...
iml Shapley  |  -             |   -             | -            |  -         | -    |  -   |
                    ...
                    select variables based on RF impurity measure
...
```
```{R , echo=FALSE, eval=TRUE, result='asis',fig.cap='...'}
...
shap <- explain(fit, X = subset(mtcars, select = -mpg), nsim = 10,  pred_wrapper = pfun)
head(shap)

apply(shap,2,f<-function(x){mean(abs(x))})



shap <- explain(rfModel2, X = subset(data, select = -y), nsim = 10,  pred_wrapper = pfun)
head(shap)

SHAP.global  <- apply(shap,2,f<-function(x){mean(abs(x))})

palette(topo.colors(n = 7))
plot(1:508,SHAP.global,type="p",col=col,pch=16,cex=0.8,
     xlab="variable number",ylab="log importances")
lines(1:508,SHAP.global,col = "gray62",lwd=0.5)
abline(v=127,col="green")
