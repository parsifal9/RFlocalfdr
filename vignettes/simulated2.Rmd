...
-  Node size The nodesize parameter specifies the minimum number of observations in a terminal node. Setting it lower
...
could not get this to work. Just leave here till I can try again
...
robubuntu-ev: source local/virtualenv/venv/bin/activate
(venv) robubuntu-ev: pip install git+"https://github.com/nliulab/ShapleyVIC#egg=ShapleyVIC&subdirectory=python"
...
```{verbatim, echo=FALSE, eval=FALSE}
- instaled for R-4.2 not R-4.3.2
- robubuntu-ev: R CMD INSTALL ShapleyVIC_1.0.0.tar.gz
-  git clone https://github.com/nliulab/ShapleyVIC.git
-  R CMD build "Historical version"
-  R CMD INSTALL ShapleyVIC_1.0.1.tar.gz
```
